...
- **Shared Obsidian Brain — full operating contract (Co-CEO 2026-06-27 memo, supersedes 2026-06-26 entry).** Path: `/Users/brassfieldventuresllc/Claude/Brain/Brain/wiki/`. Markdown files = sole source of truth (index/embedding/graph are downstream + rebuildable). Mavis vault MIRRORS the Brain, never forks. **Soft-delete only** — prefer redirect stub over deletion (git-style history).
  - **Reading contract — check Brain BEFORE answering any world-knowledge question.** `brain-think` = relational/synthesis/"how/why" (graph-first → cited answer → honest gap report; use when answer spans >1 page). Raw search = direct lookup of one fact. Never answer from memory without checking. If the Brain doesn't know, **say so — don't fabricate**.
  - **Writing contract — 3 registers per page:** (1) **Compiled-truth body** — present tense, rewritten in place when evidence changes. No dated sprawl. (2) **## Timeline** — dated, append-only history. Never edit a past entry. Format: `- YYYY-MM-DD — what changed ^[provenance]`. Add entry only when compiled truth actually changes. (3) **## Edges** — typed links, one `predicate:: [[target-slug]]` per line. Direction = this-page → target. Typed edge is also a normal wikilink — broken target breaks link-health.
  - **Closed 12-predicate vocabulary (DON'T invent):** `works_at` · `founded` · `advises` · `invested_in` · `attended` · `mentions` · `implements` · `gated_by` · `depends_on` · `part_of` · `supersedes` · `contradicts`.
  - **Edge writing = zero-LLM / pattern-matchable.** Write edges ONLY when the relationship is obvious from the page text. **Never run an LLM to "discover" relationships.** If not obvious → plain `[[wikilink]]` or `mentions::`. ≥2 outbound `[[wikilinks]]` per page (real wikilinks wire the graph + give free backlinks). Every `[[target]]` must resolve.
  - **Brain vs Memory (never cross streams).** Brain = durable world knowledge (a fact about a person goes on their page, even "Pedro prefers email"). Agent memory = operational/preference/session state. On any shared fact, the Brain WINS.
  - **Nightly dream cycle.** Research Hermes (overnight) = source ingestion + page writes + typed edges. Dream-cycle cron = consolidation, runs 02:00 local (LIVE, ~02:08 after jitter). **Honest caveat: app-open-gated** — runs while desktop app is open or on next launch. NOT an OS daemon. **Auto-run (deterministic, token-free, unambiguous fixes auto-apply):** graph scan, link-health, staleness, orphans, integrity. **HITL-gated proposals (NEVER auto-applied):** contradiction detection, dedup/merge, salience, gap sweep, tomorrow-prep. No merge/rewrite without human release.
  - **Rules of engagement — Do:** check Brain first · write what you learn back as pages (compiled body + Timeline on real changes + typed Edges) · link with real `[[wikilinks]]`, ≥2 per page · route operational state to memory · bump `updated:` + update index/log. **Don't:** answer from memory without checking · LLM-invent relationships · invent predicates outside the 12 · let the body sprawl (history → Timeline) · apply merges/dedups yourself (queue for HITL) · fork the Brain in Mavis's vault.
  - **Mavis role boundary (Co-CEO 2026-06-26, reaffirmed 2026-06-27):** execution hygiene + cross-vault consistency. Hermes = operational layer + scanner enforcement. Claude (Co-CEO) = strategy + final content decisions. **Additive only** — never delete wiki content without Founder approval; backtick-escape `\[\[wikilinks\]\]` in skill-prose describing syntax. Load skills by Read-tool on the file (NOT via `mavis skill` — these are wiki-resident, not in my registry; no-wrappers rule applies).
  - **Trigger pattern:** touch wiki content → `wiki-page-authoring` + `index-and-log-maintenance` together · end any wiki task → `wikilink-health` (use the file's backtick-aware scanner, not naive `grep '\[\['`) · bi-weekly or after 5+ page batch → `brain-improvement-pass`. Four skills + umbrella: `wiki-page-authoring`, `wikilink-health`, `index-and-log-maintenance`, `brain-improvement-pass`, plus `brain-wiki` umbrella.
